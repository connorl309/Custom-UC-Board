/**
 *******************************************************************************
 * @file    main.c
 * @brief   the application functions of AES_ECB demo for the TOSHIBA
 *          'TMPM46B' Device Series
 * @version V0.100
 * @date    2015/02/27
 *
 * DO NOT USE THIS SOFTWARE WITHOUT THE SOFTWARE LICENSE AGREEMENT.
 *
 * (C)Copyright TOSHIBA ELECTRONIC DEVICES & STORAGE CORPORATION 2017 All rights reserved
 *******************************************************************************
 */

/* Includes ------------------------------------------------------------------*/
#include "main.h"

/* Global variables ----------------------------------------------------------*/
uint32_t PlainText[3U][4U] = {
    {0x11111111U, 0x22222222U, 0x33333333U, 0x44444444U},
    {0x55555555U, 0x66666666U, 0x77777777U, 0x88888888U},
    {0x11223344U, 0x55667788U, 0x12123434U, 0x56567878U}
};
uint32_t EncryptedText[3U][4U] = {
    {0x00000000U, 0x00000000U, 0x00000000U, 0x00000000U},
    {0x00000000U, 0x00000000U, 0x00000000U, 0x00000000U},
    {0x00000000U, 0x00000000U, 0x00000000U, 0x00000000U}
};
uint32_t DecryptedText[3U][4U] = {
    {0x00000000U, 0x00000000U, 0x00000000U, 0x00000000U},
    {0x00000000U, 0x00000000U, 0x00000000U, 0x00000000U},
    {0x00000000U, 0x00000000U, 0x00000000U, 0x00000000U}
};
uint32_t EncryptionKey[6U] = {
    0x11223344U, 0x44332211U, 0x55667788U,
    0x12123434U, 0x43432121U, 0x56567878U
};
uint32_t DecryptionKey[6U] = {
    0x00000000U, 0x00000000U, 0x00000000U,
    0x00000000U, 0x00000000U, 0x00000000U
};
volatile uint32_t CompleteFlag = 0U;

/* Private functions ---------------------------------------------------------*/
/* Main function */
int main(void)
{
    Result AESResult = ERROR;
    uint32_t i = 0U;

    /* Configure UART */
    hardware_init(UART_RETARGET);

    /* Enable DMA interrupts */
    NVIC_ClearPendingIRQ(INTDMACTC8_IRQn);
    NVIC_EnableIRQ(INTDMACTC8_IRQn);
    NVIC_ClearPendingIRQ(INTDMACTC9_IRQn);
    NVIC_EnableIRQ(INTDMACTC9_IRQn);
    __enable_irq();

    /* Initialze WDMA, WDMA will be triggered by DMA transfer interrupt. */
    AES_WDMAInit(AES_ENCRYPTION_MODE);
    /* Initialzie RDMA, RDMA will be triggered by AES calculation complete. */
    AES_RDMAInit(AES_ENCRYPTION_MODE);
    /* Initialize AES to encryption mode */
    AES_Config(AES_ENCRYPTION_MODE);
    CompleteFlag = 0U;
    /* Encrypt 3 unit's plaintext. */
    while (CompleteFlag != 1U) {
        AES_SetDMAState(ENABLE);
    }
    /* Get the output key */
    for (i = 0U; i < 6U; i++) {
        DecryptionKey[i] = AES_GetKey(i + 2U);
    }

    /* Initialze WDMA, WDMA will be triggered by DMA transfer interrupt. */
    AES_WDMAInit(AES_DECRYPTION_MODE);
    /* Initialzie RDMA, RDMA will be triggered by AES calculation complete. */
    AES_RDMAInit(AES_DECRYPTION_MODE);
    /* Initialize AES to decryption mode */
    AES_Config(AES_DECRYPTION_MODE);
    CompleteFlag = 0U;
    /* Decrypt the 3 unit's encryted text. */
    while (CompleteFlag != 1U) {
        AES_SetDMAState(ENABLE);
    }

    /* Compare plaintext with decrypted text to verify the AES ECB function. */
    AESResult = TextCompare(PlainText, DecryptedText, 3U, 4U);

    if (AESResult == SUCCESS) {
        common_uart_disp("AES ECB mode processed successfully !\n");
    } else {
        common_uart_disp("AES ECB mode processed with error !\n");
    }

    while(1);
}

#ifdef DEBUG
void assert_failed(char *file, int32_t line)
{
    while (1) {
        __NOP();
    }
}
#endif

/*********************************** END OF FILE ******************************/
